...
def render_subject(subject_id: str) -> Path:
    ...
    # V5 ISOLATION: output ke cache/v5/_pdf_out/ — JANGAN overwrite production #result/
    safe_name = name.replace(" ", "_")
    safe_hanzi = (hanzi or "").replace("?", "").strip() or "TBD"
    pdf_name = f"{safe_name}-{safe_hanzi}-{birth}-V5.pdf"
    out_pdf = ROOT / "_pdf_out" / today / pdf_name
