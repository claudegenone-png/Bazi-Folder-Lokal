...
# 2026-05-29: OneDrive kadang bikin mtime .pyc kacau → Python pakai bytecode BASI
# (edit engine tidak kebaca saat build). Hapus __pycache__ + matikan tulis bytecode
# supaya hasil build SELALU mencerminkan source engine terbaru.
sys.dont_write_bytecode = True
try:
    import shutil as _sh
    for _r, _ds, _fs in __import__("os").walk(str(ROOT)):
        if "__pycache__" in _ds:
            _sh.rmtree(__import__("os").path.join(_r, "__pycache__"), ignore_errors=True)
except Exception:
    pass
...
def notify_done(title: str, message: str):
    ...
    try:
        import subprocess
        # Sanitize quotes in message for PS command line
        t = title.replace('"', "'")
...
def build_dmcard_html(subject: dict) -> str:
    ...
    pinyin = PINYIN[stem]
    strength = dm.get("strength_hz") or "旺"
    if strength == "旺":
        strength_id = "KUAT"
    elif strength == "平":
        strength_id = "SEIMBANG"
    else:
        strength_id = "LEMAH"
    # FULL-MD strict: NO template traits prose (foto tidak punya 3-trait per stem)
    return (
        '\n        <div class="dmc-eyebrow">本 命 · Penguasa Hari</div>\n'
        '        <div class="dmc-stem">' + stem + '</div>\n'
        '        <div class="dmc-element">' + el_hz + ' · ' + el_id + ' ' + polarity + '</div>\n'
        '        <div class="dmc-pinyin">' + pinyin + ' ' + el_hz + ', "' + profile['label'] + '"</div>\n'
        '        <div class="dmc-strength">Status: <span class="val">' + strength_id + '</span> · ' + strength + '</div>\n      '
    )
...
def main():
    ...
    # ---------- Step 0.5: Preflight validation (V7.1) ----------
    try:
        import subprocess, os
        env = os.environ.copy()
        env["PYTHONIOENCODING"] = "utf-8"
        rc = subprocess.run(
            [sys.executable, str(ROOT / "preflight.py"), subject_id],
            env=env, cwd=str(ROOT)
        ).returncode
        if rc == 2:
            print("\n=== ABORT: preflight FAILED — fix MD errors above. Build NOT started. ===\n")
            sys.exit(2)
        # rc==1 (warnings) or rc==0 → proceed
    except FileNotFoundError:
        # preflight.py not present, skip silently
        pass

    # ---------- Step 0.6: BaZi cross-check (shio/pilar/da_yun matematis) ----------
    try:
        vb_path = ROOT / "validate_bazi.py"
        if vb_path.exists():
            import subprocess, os
            env2 = os.environ.copy()
            env2["PYTHONIOENCODING"] = "utf-8"
            rc2 = subprocess.run(
                [sys.executable, str(vb_path), subject_id],
                env=env2, cwd=str(ROOT)
            ).returncode
            if rc2 == 2:
                print("\n=== ABORT: validate_bazi FAILED — pilar/da_yun mismatch, fix MD. ===\n")
                sys.exit(2)
    except Exception:
        pass
    ...
    for key, slug_prefix in [("zheng_cai", "caifu_zheng"), ("pian_cai", "caifu_pian")]:
        card = cf.get(key) or {}
        pct_raw = (card.get("percent") or "").strip()
        if pct_raw:
            pct = pct_raw
            import re as _re
            ...
            tafsir_blocks[f"{slug_prefix}_gauge"] = '<div class="cfm-gauge-fill" style="display:none"></div>'
        if card.get("body"):
    ...
    for page_key, struct_key, palaces in PALACE_PAGES:
        ...
        for slug, hz, pinyin, id_label in palaces:
            ...
            if insight:
                if "||" in insight:
                    paras = [p.strip() for p in insight.split("||") if p.strip()]
                    body_inner = "".join(f"<p>{_md_inline(p)}</p>" for p in paras)
                else:
                    body_inner = f"<p>{_md_inline(insight)}</p>"
                insight_html = f'<div class="pal-body">{body_inner}</div>'
            else:
                insight_html = ""
            action_html = ""
            if action:
            ...
            # SOLO palace pages (1 istana per page): separate body + action blocks with adaptive font
            if insight:
                if "||" in insight:
                    solo_paras = [p.strip() for p in insight.split("||") if p.strip()]
                    solo_body = "".join(f"<p>{_md_inline(p)}</p>" for p in solo_paras)
                else:
                    solo_body = f"<p>{_md_inline(insight)}</p>"
                import re as _re_solo
                plain_len = len(_re_solo.sub(r"<[^>]+>", "", solo_body))
                if plain_len > 3500:
                    style_override = (
                        '<style>.palace-solo .ps-body p { font-size: 6.8pt; line-height: 1.32; margin: 0 0 1.4mm 0; }'
                        '.palace-solo .ps-action, .palace-solo .ps-meta-row { display: none; }'
                        '.palace-solo { padding: 4mm 7mm; gap: 2mm; grid-template-rows: auto minmax(0, 1fr); }'
                        '.palace-solo .ps-title-id { font-size: 18pt; }'
                        '.palace-solo .ps-title-hz { font-size: 16pt; }</style>'
                    )
                elif plain_len > 2500:
                    style_override = '<style>.palace-solo .ps-body p { font-size: 8pt; line-height: 1.4; margin: 0 0 1.8mm 0; }</style>'
                elif plain_len > 1500:
                    style_override = '<style>.palace-solo .ps-body p { font-size: 9pt; line-height: 1.48; margin: 0 0 2.2mm 0; }</style>'
                else:
                    style_override = ""
                if not action:
                    style_override += '<style>.palace-solo .ps-action{display:none!important}</style>'
                tafsir_blocks[f"_solo_{slug}_body"] = style_override + solo_body
            if action:
                tafsir_blocks[f"_solo_{slug}_action"] = _md_inline(action)

    def _html_amp(s: str) -> str:
        return (s or "").replace("&", "&amp;")

    # Kesimpulan NARRATIVE (foto-synthesized long prose, anchored on 命宫 + supplementary)
    # Field comes from MD `kesimpulan_narrative` (single-line dengan || paragraph separator)
    kes_narr = subject.get("kesimpulan_narrative")
    if kes_narr:
        if "||" in kes_narr:
            kes_paras = [p.strip() for p in kes_narr.split("||") if p.strip()]
            kes_body = "".join(f"<p>{_md_inline(p)}</p>" for p in kes_paras)
        else:
            kes_body = f"<p>{_md_inline(kes_narr)}</p>"
        # Adaptive font similar to palace solo
        import re as _re_kes
        plain_len = len(_re_kes.sub(r"<[^>]+>", "", kes_body))
        if plain_len > 5500:
            style_override = '<style>.kes-narr .kn-body p { font-size: 7.5pt; line-height: 1.4; margin: 0 0 1.8mm 0; } .kes-narr { padding: 4mm 6mm; gap: 2mm; } .kn-title-id { font-size: 18pt; } .kn-title-hz { font-size: 16pt; }</style>'
        elif plain_len > 4000:
            style_override = '<style>.kes-narr .kn-body p { font-size: 8.5pt; line-height: 1.45; margin: 0 0 2mm 0; }</style>'
        else:
            style_override = ''
        tafsir_blocks["_kes_narrative"] = style_override + kes_body
    ...
    # ShenSha trio cards (page 8): 3 equal cards from shen_sha_list (foto verbatim)
    ssl = subject.get("shen_sha_list") or []
    STAR_INFO = {
        "驛馬": {"pinyin": "Yì Mǎ", "id": "Bintang Perpindahan",
            "foto_zh": "勞祿好動, 奔波遠行, 多旅行運, 住家及事業多變動.",
            "foto_id": "Aktif & gemar bergerak, perjalanan jauh, keberuntungan dari travel, rumah & pekerjaan banyak berubah."},
        "文昌貴": {"pinyin": "Wén Chāng Guì", "id": "Bintang Akademik Mulia",
            "foto_zh": "聰明過人, 機謀多變之人, 逢凶化吉.",
            "foto_id": "Pintar luar biasa, banyak akal & strategi adaptif, mampu mengubah kemalangan jadi keberuntungan."},
        "天醫": {"pinyin": "Tiān Yī", "id": "Bintang Tabib",
            "foto_zh": "在醫界一定成名, 為人敬仰, 可作良醫.",
            "foto_id": "Pasti tersohor di bidang medis, dihormati orang, layak jadi tabib yang baik."},
    }
    if ssl:
        cards_html = []
        for star_item in ssl:
            star_hz = star_item.get("hanzi") if isinstance(star_item, dict) else str(star_item)
            if not star_hz:
                continue
            info = STAR_INFO.get(star_hz, {"pinyin": "", "id": "—", "foto_zh": "—", "foto_id": "—"})
            cards_html.append(
                f'<div class="sst-card">'
                f'<div class="sst-hz">{star_hz}</div>'
                f'<div class="sst-meta"><div class="sst-id">{info["id"]}</div><div class="sst-pinyin">{info["pinyin"]}</div></div>'
                f'<div class="sst-body"><strong>Foto verbatim:</strong> <span class="hz">{info["foto_zh"]}</span><br><br>{info["foto_id"]}</div>'
                f'</div>'
            )
        tafsir_blocks["shensha_trio_cards"] = "".join(cards_html)
    ...
    if no_pdf:
        ...
        notify_done(
            f"V7 HTML render OK · {subject_id}",
            f"{time.time()-t0:.1f}s · refresh browser ke _master.html"
        )
    else:
        ...
        # Post-render preflight: scan for ** literal leak in HTML
        _run_postrender_check(subject_id)



def _run_postrender_check(subject_id: str):
    """Scan rendered HTML for ** literal leak (preflight post-render)."""
    try:
        import subprocess as _sp, os as _os
        env = _os.environ.copy()
        env["PYTHONIOENCODING"] = "utf-8"
        _sp.run(
            [sys.executable, str(ROOT / "preflight.py"), subject_id, "--post-render"],
            env=env, cwd=str(ROOT)
        )
    except Exception:
        pass
