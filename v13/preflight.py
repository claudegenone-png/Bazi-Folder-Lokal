...
def check_palace_completeness(data: dict) -> list[str]:
    ...
    warnings = []
    SHORT = 300  # char; di bawah ini insight dicurigai diringkas (naik dari 220, 2026-06-21)
    present = 0
...
def check_blueprint_compliance(data: dict, md_text: str) -> tuple[list[str], list[str]]:
    """5 checks untuk enforce MD Blueprint format — mencegah render errors dari format salah.
    Semua check ini meng-catch error yang tidak terlihat dari PDF (build tetap sukses)."""
    errors, warnings = [], []

    # 1. dm_pos_score / dm_neg_score harus integer >= 100
    for key in ("dm_pos_score", "dm_neg_score"):
        v = data.get(key)
        if v is not None:
            try:
                ...
                if num < 100:
                    errors.append(
                        f"{key}={v!r} — engine expect INTEGER e.g. 4100 (bukan +4.100). "
                        f"float < 100 = format salah, DM score akan print angka truncated. "
                        f"Ganti ke: {key}: 4100"
                    )
            except (ValueError, TypeError):
                pass

    # 2. liu_nian_YYYY harus punya dua | separator (umur|ganzhi|prose)
    for line in md_text.splitlines():
        stripped = line.strip()
        if not stripped.startswith("- liu_nian_"):
            continue
        ...
        if val and val.lower() not in ("null", "none", ""):
            if val.count("|") < 2:
                errors.append(
                    f"{key.strip()}: format salah — WAJIB dua pipe: umur|ganzhi|prose "
                    f"(e.g. 24|丙午|Tekanan tahun...). Sekarang: {val[:60]!r}"
                )

    # 3. yang_zhai_zone_* ada tapi yang_zhai_gua kosong → halaman YangZhai skip
    yang_zhai_gua = data.get("yang_zhai_gua")
    zone_keys = [k for k in data if k.startswith("yang_zhai_zone_") and data[k]]
    if zone_keys and not yang_zhai_gua:
        errors.append(
            f"yang_zhai_zone_* ada ({len(zone_keys)} zona) tapi yang_zhai_gua kosong — "
            f"halaman Yang Zhai tidak akan render tanpa gua. "
            f"Tambah: - yang_zhai_gua: 艮  (sesuai foto)"
        )

    # 4. TAFSIR jie_e.insight ada tapi DATA jie_e_palace_id kosong → halaman Kesehatan kosong
    jie_e_tafsir = data.get("palace_jie_e_insight")
    jie_e_data = data.get("jie_e_palace_id")
    if jie_e_tafsir and not jie_e_data:
        errors.append(
            "TAFSIR jie_e.insight ada tapi DATA jie_e_palace_id kosong — "
            "halaman Kesehatan render dari DATA (TAFSIR saja tidak cukup). "
            "Tambah di ## DATA: - jie_e_palace_id: <terjemahan penuh dari foto>"
        )

    # 5b. bone_weight_total null → halaman 秤骨 akan di-skip tanpa warning
    bw_total = data.get("bone_weight_total")
    bw_poem_hz = data.get("bone_weight_poem_hz")
    bw_poem_id = data.get("bone_weight_poem_id")
    if not bw_total:
        warnings.append(
            "bone_weight_total null → halaman 秤骨 (Berat Tulang Nasib) akan di-skip. "
            "Kalau ada foto 先天論命 秤骨, isi bone_weight_year/month/day/hour/total di ## DATA."
        )
    elif bw_poem_hz and not bw_poem_id:
        warnings.append(
            "bone_weight_poem_hz diisi tapi bone_weight_poem_id null → "
            "bagian puisi di halaman 秤骨 tidak ada terjemahan Indonesia. Tambah bone_weight_poem_id."
        )

    # 5. Literal 'FILL' tersisa di DATA section → field belum diisi dari foto
    in_data = False
    fill_fields = []
    for line in md_text.splitlines():
        stripped = line.strip()
        ...
        if in_data and stripped.startswith("- ") and ":" in stripped:
            key, _, val = stripped[2:].partition(":")
            if val.strip() == "FILL":
                fill_fields.append(key.strip())
    if fill_fields:
        errors.append(
            f"Field masih berisi literal 'FILL' di ## DATA — wajib diisi dari foto atau set null: "
            f"{', '.join(fill_fields)}"
        )
...
def check_hanzi_labels(data: dict, md_text: str) -> list[str]:
    """HARD ERROR: setiap [[CJK-only]] dalam prose fields WAJIB punya label Indonesia
    atau pinyin dalam 50 char SEBELUM [[tag]].
    Cek: ada Latin word ≥4 char dalam prefix → PASS. Tidak ada → ERROR.
    Exception:
      - Ranting posisi tunggal 子丑寅卯辰巳午未申酉戌亥
      - Ganzhi 2-char (stem+branch) seperti [[庚午]] [[甲戌]]
    """
    BRANCH_WL  = set("子丑寅卯辰巳午未申酉戌亥")
    STEMS      = set("甲乙丙丁戊己庚辛壬癸")
    HANZI_TAG  = re.compile(r'\[\[([一-鿿㐀-䶿]{1,12})\]\]')
    LATIN4     = re.compile(r'[A-Za-z]{4,}')          # ≥4 chars = meaningful label word

    PROSE_KEYS = (
        [f"palace_{p}_insight" for p in PALACE_KEYS]
        + ["kesimpulan_narrative", "ziwei_su_ming",
           "jie_e_palace_id", "jie_e_organ_focus_id",
           "marriage_cocok_tafsir", "marriage_hindari_tafsir"]
    )

    def _is_ganzhi(hz: str) -> bool:
        return (len(hz) == 2
                and hz[0] in STEMS
                and hz[1] in BRANCH_WL)

    def _scan(text: str, field_label: str, errs: list):
        for m in HANZI_TAG.finditer(text):
            hz = m.group(1)
            # Exception: single branch position or Ganzhi combo
            if hz in BRANCH_WL or _is_ganzhi(hz):
                continue
            # Only check the 50 chars BEFORE [[
            prefix = text[max(0, m.start() - 50): m.start()]
            if not LATIN4.search(prefix):
                errs.append(
                    f"{field_label}: [[{hz}]] tidak punya label Indonesia/pinyin "
                    f"(tidak ada kata ≥4 huruf sebelumnya) "
                    f"— tulis: 'NamaLabel [[{hz}]]'"
                )
    ...
    for key in PROSE_KEYS:
        ...
        if not text or text.lower() in ("null", "none", "—", "-"):
            continue
        _scan(text, key, errors)

    # Liu nian prose (bagian setelah pipe ke-2)
    for line in md_text.splitlines():
        stripped = line.strip()
        if not stripped.startswith("- liu_nian_"):
            continue
        ...
        if len(parts) >= 3:
            prose = "|".join(parts[2:]).strip()
            if prose:
                _scan(prose, key.strip() + "_prose", errors)

    return errors


def check_liunian_opening_uniqueness(md_text: str) -> list[str]:
    """Warn kalau 2+ tahun liu_nian punya kalimat pembuka identik (RULE 10).
    Software BaZi sering pakai template text yang sama untuk tahun-tahun serupa —
    translator wajib parafrase per tahun agar tidak identik."""
    def first_n_words(text: str, n: int = 10) -> str:
        words = re.sub(r'[^\w\s]', ' ', text).split()[:n]
        return ' '.join(w.lower() for w in words)

    entries: dict[str, list[str]] = {}  # normalized_opening -> [year_keys]
    for line in md_text.splitlines():
        stripped = line.strip()
        if not stripped.startswith("- liu_nian_"):
            continue
        ...
        if len(parts) >= 3:
            prose = "|".join(parts[2:]).strip()
            if prose:
                opening = first_n_words(prose)
                entries.setdefault(opening, []).append(key.strip())

    warnings = []
    for opening, years in entries.items():
        if len(years) > 1:
            warnings.append(
                f"liu_nian kalimat pertama IDENTIK di: {', '.join(years)} — "
                f"RULE 10 VIOLATION: parafrase pembuka tiap tahun dari sudut pandang berbeda "
                f"(frasa: '{opening[:60]}')"
            )
    return warnings


def check_nama_format(data: dict) -> list[str]:
    """Warn kalau nama subjek mengandung spasi — kemungkinan copy dari software
    (e.g. 'BA YU' padahal nama asli 'BAYU'). Perlu konfirmasi ke user."""
    nama = (data.get("nama") or "").strip()
    if ' ' in nama:
        return [
            f"nama '{nama}' mengandung spasi — konfirmasi ke user: "
            f"apakah memang '{nama}' atau tanpa spasi? "
            f"(software BaZi sering tampilkan nama dengan spasi)"
        ]
    return []


def check_palace_name_personalization(data: dict) -> list[str]:
    """WARN kalau palace insight tidak menyebut nama subjek sama sekali.
    RULE 6: setiap palace insight wajib sebut nama subjek minimal 1x."""
    nama = (data.get("nama") or "").strip()
    ...
    for pal in PALACE_KEYS:
        ins = (data.get(f"palace_{pal}_insight") or "").strip()
        if not ins or len(ins) < 50:
            continue  # null atau sangat pendek, skip
        if nama_lower not in ins.lower():
            warnings.append(
                f"istana '{pal}' insight tidak menyebut nama subjek '{nama}' "
                f"(RULE 6: wajib sebut nama minimal 1x, e.g. '{nama} adalah...' atau 'Bagi {nama}...')"
            )
    return warnings


def check_ming_gong_completeness(data: dict) -> list[str]:
    """ERROR (bukan warning) kalau palace_ming_gong_insight terlalu pendek.
    Sumber render halaman 命宮 = TAFSIR palace_detail_1 → ming_gong.insight.
    parse_md_full() menyimpan TAFSIR insight ke palace_ming_gong_insight (overwrite DATA).
    Layar 命宮 詳細解說 biasanya sangat panjang — verbatim penuh ≥700 char.
    Di bawah threshold = kemungkinan diringkas → BUILD STOP (upgraded 2026-06-21)."""
    ERROR_THRESHOLD = 700
    ins = (data.get("palace_ming_gong_insight") or "").strip()
    if not ins or ins.lower() in ("null", "none", "—", "-"):
        return []  # kosong sudah ditangani check_critical_fields
    if len(ins) < ERROR_THRESHOLD:
        return [
            f"CRITICAL: palace_ming_gong_insight ({len(ins)} char) di bawah minimum ≥{ERROR_THRESHOLD} char — "
            f"TAFSIR ming_gong.insight DIRINGKAS (render 命宮 = TAFSIR, bukan DATA). "
            f"WAJIB terjemah PENUH per-klausa dari foto 命宮 詳細解說, klausa ≥95%. BUILD STOP."
        ]
    return []


def check_sekilas_hidup_format(md_text: str) -> list[str]:
    """ERROR kalau ### Sekilas Hidup ditulis format inline 'card: Label | teks'
    bukan list '- Label | teks'. Parser hanya baca bullets — inline = halaman kosong."""
    in_section = False
    bullet_count = 0
    inline_count = 0
    for line in md_text.splitlines():
        stripped = line.strip()
        if stripped == "### Sekilas Hidup":
            in_section = True
            bullet_count = 0
            inline_count = 0
            continue
        ...
        if in_section:
            if re.match(r"^\s*-\s+\S.*\|", line):
                bullet_count += 1
            elif re.match(r"^\s*card\s*:\s*\S", line):
                inline_count += 1
    if inline_count > 0 and bullet_count == 0:
        return [
            f"### Sekilas Hidup: {inline_count} card ditulis format INLINE 'card: Label | teks' "
            f"— parser hanya baca '- Label | teks' bullets, halaman 全局總論 akan KOSONG. "
            f"Fix: ubah ke 'card:' (baris sendiri) lalu tiap card '- Label | teks'"
        ]
    return []
...
def run(subject_id: str, post_render: bool = False) -> int:
    if post_render:
        ...
        print(f"\n[preflight V11 — post-render] subject_id={subject_id}")
        if issues:
            # Group by category
            by_cat = {}
    ...
    errors += check_hanzi_labels(data, md_text)
    warnings += check_wuxing_sum(data)
    warnings += check_shio_year_branch(data)
    warnings += check_palace_completeness(data)
    errors += check_ming_gong_completeness(data)  # ERROR (bukan warning) — 2026-06-21
    warnings += check_liunian_completeness(md_text)
    e, w = check_blueprint_compliance(data, md_text); errors += e; warnings += w
    errors += check_sekilas_hidup_format(md_text)
    warnings += check_palace_name_personalization(data)
    warnings += check_liunian_opening_uniqueness(md_text)
    warnings += check_nama_format(data)
    # Career page check: warn kalau tidak ada data industri (halaman karir akan di-skip)
    if not data.get("shiye_favorable_full") and not data.get("shiye_supportive_full"):
        warnings.append("shiye_favorable_full + shiye_supportive_full keduanya null → halaman Karir di-skip. Isi dari foto 事業 kalau ada.")

    print(f"\n[preflight V11] subject_id={subject_id}")
