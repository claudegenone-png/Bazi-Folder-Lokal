...
def build_subject_from_ocr(subject_id: str,
    ...
    if gua_hz and gua_hz in TRIGRAMS:
        t = TRIGRAMS[gua_hz]
        _dir_hz  = {"N":"北","S":"南","E":"東","W":"西","NE":"東北","SW":"西南","NW":"西北","SE":"東南"}
        _dir_ab  = {"N":"U","S":"S","E":"T","W":"B","NE":"TL","SW":"BD","NW":"BL","SE":"TG"}
        _opp_hz  = {"N":"南","S":"北","E":"西","W":"東","NE":"西南","SW":"東北","NW":"東南","SE":"西北"}
        _opp_ab  = {"N":"S","S":"U","E":"B","W":"T","NE":"BD","SW":"TL","NW":"TG","SE":"BL"}

        # sumbu_hoki = personal gua axis (for 本命卦 card)
        personal_pos = t["pos"]
        sumbu_hoki_hz = f"{_dir_hz[personal_pos]}↔{_opp_hz[personal_pos]}"
        sumbu_hoki_id = f"{_dir_ab[personal_pos]} ↔ {_opp_ab[personal_pos]}"

        # sumbu_rumah = house orientation axis (for compass SVG) — from zone_rumah_hz
        zone_rumah_raw = ocr.get("yang_zhai_zone_rumah_hz") or ""
        _zone_order = [("東北","NE"),("西北","NW"),("東南","SE"),("西南","SW"),
                       ("南","S"),("北","N"),("東","E"),("西","W")]
        rumah_axis_pos = personal_pos  # fallback to personal gua if house data missing
        for k, v in _zone_order:
            if k in zone_rumah_raw:
                rumah_axis_pos = v
                break
        sumbu_rumah_hz = f"{_dir_hz[rumah_axis_pos]}↔{_opp_hz[rumah_axis_pos]}"
        sumbu_rumah_id = f"{_dir_ab[rumah_axis_pos]} ↔ {_opp_ab[rumah_axis_pos]}"
        ...
        yang_zhai = {
            ...
            "group_id": t["group"],
            "desc_id": t.get("desc_id", ""),
            # personal gua axis — used by 本命卦 side card
            "sumbu_hoki_hz": sumbu_hoki_hz,
            "sumbu_hoki_id": sumbu_hoki_id,
            # house orientation axis — used by compass SVG
            "sumbu_rumah_hz": sumbu_rumah_hz,
            "sumbu_rumah_id": sumbu_rumah_id,
            "rumah_axis_pos": rumah_axis_pos,
        ...
        if yz_zones_foto:
            ...
            # Fallback: TAFSIR zones: list (label/note format) → convert to dict keyed by zone key
            _TAFSIR_LABEL_TO_KEY = {
                "pintu utama": "pintu", "pintu": "pintu",
                "kamar tidur": "kamar", "kamar & ranjang": "kamar", "kamar": "kamar",
                "dapur": "dapur", "dapur / kompor": "dapur",
                "kamar mandi": "kamar_mandi", "kamar mandi / wc": "kamar_mandi",
                "altar": "altar", "altar / sembah": "altar",
                "orientasi rumah": "rumah", "rumah": "rumah",
            }
            tafsir_zones = (ocr.get("_yangzhai_struct") or {}).get("zones") or []
            if tafsir_zones:
                zones_dict = {}
                for z in tafsir_zones:
                    label = (z.get("label") or "").strip().lower()
                    key = _TAFSIR_LABEL_TO_KEY.get(label)
                    if key:
                        zones_dict[key] = {"note_id": z.get("note", ""), "dirs_hz": [], "dirs_abbr": []}
                if zones_dict:
                    yang_zhai["zones"] = zones_dict

    # ---- Zi Wei (Zone 10: MD pull, py-iztro engine REMOVED) ----
    zw_in = ocr.get("zi_wei")
    zw_out = None
    if isinstance(zw_in, dict) and zw_in:
        ming = zw_in.get("ming_gong_hz")
        shen = zw_in.get("shen_gong_hz")
    ...
    out = {
        ...
        # ===== BAGUA 八卦廿四山方位圖 (optional) =====
        "bagua_sectors":     ocr.get("bagua_sectors"),
        "bagua_life_type_hz": ocr.get("bagua_life_type_hz"),
        "bagua_life_type_id": ocr.get("bagua_life_type_id"),
        "bagua_nayin_hz":    ocr.get("bagua_nayin_hz"),
        "bagua_nayin_id":    ocr.get("bagua_nayin_id"),
        "bagua_taisui_hz":   ocr.get("bagua_taisui_hz"),
        "bagua_taisui_id":   ocr.get("bagua_taisui_id"),
        "bagua_warning_hz":  ocr.get("bagua_warning_hz"),
        "bagua_warning_id":  ocr.get("bagua_warning_id"),
        "bagua_virtue_hz":   ocr.get("bagua_virtue_hz"),
        "bagua_virtue_id":   ocr.get("bagua_virtue_id"),
        "bagua_zuo_xiang_hz": ocr.get("bagua_zuo_xiang_hz"),
        "bagua_zuo_xiang_id": ocr.get("bagua_zuo_xiang_id"),
        # ===== HONGSHUI 陽宅吉凶鑑定 text pages (optional, per-page) =====
        "hongshui_pages":    ocr.get("hongshui_pages"),
    }
