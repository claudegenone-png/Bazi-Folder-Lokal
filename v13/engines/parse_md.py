...
# Auto-translate table: Yang Zhai _note Hanzi → Indonesian.
# Applied if the note field still contains CJK characters.
# Ordered longest-first to avoid partial-match collisions.
_YZ_SUBS = [
    # Zone prefixes
    ("坑廁宜安於", "Kamar mandi/WC sebaiknya di "),
    ("坑廁宜安",   "Kamar mandi/WC sebaiknya di "),
    ("爐灶宜安",   "Kompor sebaiknya di "),
    ("門路宜開",   "Pintu baik ke "),
    ("宅宜坐",    "Rumah sebaiknya duduk di "),
    ("宅位宜安",  "Posisi rumah sebaiknya di "),
    ("房間宜",    "Kamar tidur baik di "),
    ("床位宜安",  "Posisi ranjang sebaiknya di "),
    ("床位",      "Posisi ranjang baik di "),
    ("神位安",    "Altar sebaiknya di "),
    # Multi-char modifiers (before single 吉)
    ("本年大利方合吉", ", serta arah keberuntungan tahun ini"),
    ("本年大利方",     ", dan arah keberuntungan tahun ini"),
    ("大吉",   ", sangat menguntungkan"),
    ("合吉",   " menguntungkan"),
    # Compound directions — 4-char before 2-char before 1-char
    ("東南方", "Tenggara"), ("西南方", "Barat Daya"),
    ("東北方", "Timur Laut"), ("西北方", "Barat Laut"),
    ("東南",   "Tenggara"), ("西南",   "Barat Daya"),
    ("東北",   "Timur Laut"), ("西北",  "Barat Laut"),
    ("東方",   "Timur"), ("西方",   "Barat"),
    ("南方",   "Selatan"), ("北方",   "Utara"),
    ("東",     "Timur"), ("西",     "Barat"),
    ("南",     "Selatan"), ("北",    "Utara"),
...
def _autofix_yz_note(val: str) -> str:
    """Auto-translate yang_zhai _note from Hanzi to Indonesian if CJK detected.
    Safe no-op if already Indonesian. Applied at parse time so agents can write
    Hanzi directly from foto and still get correct Indonesian in the PDF."""
    if not re.search(r'[一-鿿]', val):
        return val
    result = val
    for hz, idn in _YZ_SUBS:
        result = result.replace(hz, idn)
    # Clean up: collapse multiple spaces, trailing/leading commas
...
def parse_data_section(data_text: str, h1: str, warnings: list[str]) -> dict:
    ...
    # Format A (pipe): "hanzi|Indonesia, hanzi|Indonesia, ..."
    # Format B (plain): "teks plain Indonesia, teks plain, ..." (fallback dari ziwei_career_*_id)
    def _parse_industri(v):
        items = []
        for chunk in v.split(","):
            chunk = chunk.strip()
            if not chunk:
    ...
    for k_md, k_out in [
        ...
        v = raw.get(k_md)
        if v:
            items = _parse_industri(v)
            if items:
                out[k_out] = items

    # Fallback: kalau shiye_favorable_full / shiye_supportive_full tidak ada,
    # gunakan ziwei_career_recommended_id dan ziwei_career_alternate_id
    if not out.get("shiye_favorable_full"):
        v = raw.get("ziwei_career_recommended_id") or raw.get("ziwei_career_recommended_hz")
        if v:
            items = _parse_industri(v)
            if items:
                out["shiye_favorable_full"] = items
    if not out.get("shiye_supportive_full"):
        v = raw.get("ziwei_career_alternate_id") or raw.get("ziwei_career_alternate_hz")
        if v:
            items = _parse_industri(v)
            if items:
                out["shiye_supportive_full"] = items
    ...
    for k_md, k_out in [("marriage_cocok","cocok_branches"),
        ...
        # Fallback: beberapa MD nulis marriage_cocok_shio_hz / marriage_hindari_shio_hz
        v = raw.get(k_md) or raw.get(k_md + "_shio_hz", "")
        if v:
    ...
    if "yang_zhai_zone_rumah_hz" in raw:
        out["yang_zhai_zone_rumah_hz"] = raw["yang_zhai_zone_rumah_hz"]
        ...
        if hz_field in raw and raw[hz_field]:
            ...
            if hz_list_raw.lower() not in ("null","none",""):
                ...
                if note_field in raw and raw[note_field]:
                    z["note_id"] = _autofix_yz_note(str(raw[note_field]).strip())
                yz_zones[zone_key] = z
    if yz_zones:
    ...
    for k_md, k_out in [
        ...
        ("ziwei_ming_gong","ming_gong_hz"),
        ("ziwei_shen_gong","shen_gong_hz"),
    ...
    # ===== BAGUA 八卦廿四山方位圖 =====
    # 8 canonical sector keys. Format B (flat per property) — reliable & easy to validate.
    BAGUA_SECTOR_KEYS = ["shengqi", "tianyi", "yannian", "fuwei", "jue_ming", "wu_gui", "liu_sha", "huo_hai"]
    BAGUA_SECTOR_PROPS = ["label_hz", "label_id", "star_hz", "star_id",
                          "element_hz", "element_id", "grade_hz", "grade_id",
                          "mountains_hz", "direction_id", "content_id"]
    bagua_sectors = {}
    for sk in BAGUA_SECTOR_KEYS:
        sector = {}
        for prop in BAGUA_SECTOR_PROPS:
            v = raw.get(f"bagua_sector_{sk}_{prop}")
            if v and v.lower() not in ("null", "none", ""):
                sector[prop] = str(v).strip()
        if sector:  # only add if at least 1 field present
            bagua_sectors[sk] = sector
    if bagua_sectors:
        out["bagua_sectors"] = bagua_sectors

    # Bagua center / shared info fields
    for k_md, k_out in [
        ("bagua_life_type_hz",  "bagua_life_type_hz"),
        ("bagua_life_type_id",  "bagua_life_type_id"),
        ("bagua_nayin_hz",      "bagua_nayin_hz"),
        ("bagua_nayin_id",      "bagua_nayin_id"),
        ("bagua_taisui_hz",     "bagua_taisui_hz"),
        ("bagua_taisui_id",     "bagua_taisui_id"),
        ("bagua_warning_hz",    "bagua_warning_hz"),
        ("bagua_warning_id",    "bagua_warning_id"),
        ("bagua_virtue_hz",     "bagua_virtue_hz"),
        ("bagua_virtue_id",     "bagua_virtue_id"),
        ("bagua_zuo_xiang_hz",  "bagua_zuo_xiang_hz"),
        ("bagua_zuo_xiang_id",  "bagua_zuo_xiang_id"),
    ]:
        v = raw.get(k_md)
        if v and str(v).lower() not in ("null", "none", ""):
            out[k_out] = str(v).strip()

    # ===== HONGSHUI 陽宅吉凶鑑定 TEXT PAGES (8 pages, each optional) =====
    # Each page driven by: title_hz, title_id, source_photo, content_id, content_hz (opt)
    # Skip individual page if content_id is empty/null.
    FALLBACK_TITLE_HZ = "陽宅吉凶鑑定"
    FALLBACK_TITLE_ID = "Analisis Keberuntungan Hunian"
    hongshui_pages = {}
    for i in range(1, 9):
        idx = f"{i:02d}"
        content_id = raw.get(f"hongshui_text_{idx}_content_id", "")
        if not content_id or str(content_id).lower() in ("null", "none", ""):
            continue  # skip this page — no content
        page = {
            "title_hz":     raw.get(f"hongshui_text_{idx}_title_hz") or FALLBACK_TITLE_HZ,
            "title_id":     raw.get(f"hongshui_text_{idx}_title_id") or FALLBACK_TITLE_ID,
            "source_photo": raw.get(f"hongshui_text_{idx}_source_photo") or "",
            "content_id":   str(content_id).strip(),
            "content_hz":   raw.get(f"hongshui_text_{idx}_content_hz") or "",
        }
        # Strip null/none strings in title fields
        for fk in ("title_hz", "title_id", "source_photo", "content_hz"):
            if str(page[fk]).lower() in ("null", "none"):
                page[fk] = ""
        hongshui_pages[idx] = page
    if hongshui_pages:
        out["hongshui_pages"] = hongshui_pages
...
def _render_overview_full(body: str) -> str:
    ...
    Fallback: handles inline 'card: Label | teks' (agent error) so page never silently blank."""
    items = _extract_bullets(body)
    # Fallback: agent sometimes writes `card: Label | teks` inline instead of list bullets
    if not items:
        for line in body.splitlines():
            m = re.match(r"^\s*card\s*:\s*(.+)$", line)
            if m:
                v = m.group(1).strip()
                if v.lower() not in ("null", "none", "fill", ""):
                    items.append(v)
    cards = []
    for it in items:
        if "|" in it:
        ...
        for f in ("star", "insight", "action"):
            ...
            if mf:
                ...
                if v and not v.startswith("{") and v.lower() != "null":
                    # Auto-null placeholder insights: "[TIDAK TERSEDIA]", "[NULL]", etc.
                    if f == "insight" and v.startswith("["):
                        continue
                    d[f] = v
        if d:
            out[key] = d
