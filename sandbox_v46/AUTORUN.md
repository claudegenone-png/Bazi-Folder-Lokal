# AUTORUN V4.5 — Production Playbook (Claude-side optimization)

V4.5 = **PRODUCTION** (promoted dari sandbox2 ke v45/ pada 2026-05-02). Speed + token efficiency optimization dari sisi Claude (IDE), bukan API SDK.
...
```powershell
python v45\prep_photos.py "<photos_dir>"
```
...
```powershell
python v45\cache_check.py <photos_dir>_prepped
```
...
```
v45/data/subjects/{subject_id}.ocr.json
```
...
- V4.5 production menulis ke `#result/` **tanpa suffix** (sama format V3). Output kalau subjek sama akan overwrite V3 — sudah aturan: V4.5 = default daily.
