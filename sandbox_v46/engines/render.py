...
MICHELE = {  # name kept for backward compat; this is now LinRuYi reference
    ...
    # V4.6: Lin Ru Yi paragraph baseline per section — kunci substitusi narrative.
    # Copy VERBATIM dari template HTML (lihat templates/page_*.html footer-caption).
    # Kalau subject.narrative.<key>.id ada → ganti string ini ke teks subject.
    # Kalau tidak ada → string Michele tetap, lalu page-skip logic Task 5 yang buang halaman.
    "narrative_lin_ru_yi": {
        # page_06_daymaster (overall — wuxing balance)
        "overall_para_p06": 'Bagan elemen Anda condong ke <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">木 + 水</span> (58.8%) — Pohon besar yang banyak air. Tugas hidup: <em>salurkan ke 火 金</em> — itu cara mengubah pertumbuhan jadi karya.',
        # page_08_xingqing (personality)
        "personality_para": '<span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">辛金</span> Anda adalah <em>pedang belum ditempa</em> — kekuatan baru muncul ketika dipertemukan dengan tekanan yang layak. Hindari medan datar.',
        # page_09_family (parents/family)
        "parents_para": 'Pola "eksekutor + pasangan ekstrover + anak lembut" adalah konfigurasi <em>matang</em> — <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">家</span> Anda sudah seimbang. Tugas Anda: tidak merusaknya dengan ketegasan berlebih.',
        # page_10_shensha
        "shensha_para": 'Bagan klasik mengenal 32 bintang nasib — Anda hanya punya 1 aktif (<span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">驛馬</span>). Itu bukan miskin, melainkan <em>bagan yang fokus</em>.',
        # page_11_caifu (wealth)
        "wealth_para": 'Beri 5-10% rezeki ke filantropi/keluarga besar setiap bulan: itu <em>"air mengalir keluar"</em> yang justru menjaga aliran masuk tetap lancar — sesuai <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">喜用神 水</span> Anda.',
        # page_17_palace1 (career palace)
        "palace1_para": 'Lingkaran inti Anda — diri yang berapi, saudari yang halus, pasangan yang cerah, anak yang stabil. <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">家</span> Anda berimbang; tugasnya menjaga, bukan mengubah.',
        # page_18_palace2 (marriage palace)
        "palace2_para": '4 istana operasional menunjukkan: rezeki disiplin · kesehatan terjaga oleh <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">天梁</span> · pindah lancar · teman bermutu. Fase sekarang: aktifkan jejaring, bukan tarik diri.',
        # page_19_palace3 (fortune+health+migration palace)
        "palace3_para": '4 istana lingkaran-luar — pencapaian &amp; warisan. Anda <em>ditempa oleh orangtua optimis</em>, <em>siap panen Berkah</em>, <em>warisan properti dinamis</em>, <em>karir sebagai tokoh suara</em>.',
        # page_20_kesimpulan (overall — distilasi)
        "kesimpulan_para": 'Bagan menyajikan <em>pola</em>; halaman ini menyajikan <em>distilasi</em>. Semua diagram, tafsir, dan tabel sebelumnya — berkumpul jadi satu kalimat: <strong>Logam yang ditempa Api siap jadi pedang yang bermakna</strong>.',
        # page_career
        "career_para": 'Bila membuka usaha baru, partner ber-喜用神 火/金 (tipe komunikatif &amp; edukatif) akan lebih melengkapi daripada partner sesama 辛金 yang mengulang dominasi Logam.',
        # page_dayun (liu_nian / current cycle)
        "liu_nian_para": 'Anda sedang di fase <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">正財</span> (23–32) — pilar wealth tetap &amp; komitmen relasi. Periode 23-42 adalah jendela emas untuk membangun aset, karier mapan, dan struktur keluarga yang kokoh.',
        # page_marriage
        "marriage_para": 'Pasangan ber-shio Kelinci &amp; Kambing memberikan dukungan natural; bila pasangan dekat ber-shio Ular atau Monyet, kompensasi lewat ruang istirahat &amp; komunikasi terjadwal. Hindari pula pasangan sesama Babi (自刑 · konflik diri).',
        # page_profile (personality short)
        "profile_para": 'Penguasa Hari 辛金 (Logam Yin) dengan format 正官格 menandakan kepribadian disiplin, terstruktur, dan menjunjung tinggi norma — sangat cocok pada peran yang menuntut tanggung jawab, integritas, dan kepatuhan pada sistem.',
        # page_synthesis
        "synthesis_para": 'Laporan ini bukan vonis, melainkan <em>cermin pola</em>. Pakai sebagai peta — keputusan tetap di tangan Anda. Saat <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">文武相濟</span> tercapai, Logam menjadi pedang yang menempa karya — bukan beban yang menumpuk.',
        # page_yangzhai
        "yang_zhai_para": '<span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">坎卦</span> Anda termasuk <strong>Kelompok Timur</strong> — sumbu hoki <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">北↔南</span>. Rumah ideal: duduk Utara menghadap Selatan, atau sebaliknya.',
        # page_ziwei (overall short on ziwei page)
        "ziwei_overall_para": 'Saat ini Anda berada di <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">大限 僕役</span> (umur 56-65, Istana Pertemanan) — fokus dekade ini ada di pertemanan &amp; jejaring; <span style="font-family:\'Noto Serif TC\',serif;color:var(--red)">紫微</span> (Bintang Kaisar) di sini berarti teman-teman bermutu pemimpin.',
    },
}
...
def build_substitutions(subject: dict) -> list[tuple[str,str]]:
    ...
    # ============ V4.6 FINAL NARRATIVE (Hanzi-keyed detail_sections lookup) ============
    # Each Lin Ru Yi paragraph in template ↔ one detail_sections Hanzi key.
    # Prepended to FRONT of pairs so narrative subs run BEFORE day-master subs
    # (which would mutate baseline mid-substitute).
    detail = subject.get("detail_sections") or {}
    michele_para = MICHELE.get("narrative_lin_ru_yi", {})
    # mapping: (michele baseline key, [Hanzi keys to try in detail_sections])
    NAR_MAP_HZ = [
        ("overall_para_p06",   ["全局總論", "命格總論"]),                # page_06_daymaster
        ("personality_para",   ["性情", "命格", "個性"]),                # page_08_xingqing
        ("parents_para",       ["父母", "兄弟", "子女"]),                # page_09_family
        ("shensha_para",       ["神煞"]),                                # page_10_shensha
        ("wealth_para",        ["財富", "財帛", "田宅"]),                # page_11_caifu
        ("palace1_para",       ["官祿", "事業", "命宮"]),                # page_17_palace1
        ("palace2_para",       ["夫妻", "婚配"]),                        # page_18_palace2
        ("palace3_para",       ["福德", "疾厄", "遷移"]),                # page_19_palace3
        ("kesimpulan_para",    ["全局總論"]),                            # page_20_kesimpulan
        ("career_para",        ["官祿", "事業"]),                        # page_career
        ("liu_nian_para",      ["流年判斷"]),                            # page_dayun
        ("marriage_para",      ["夫妻", "婚配"]),                        # page_marriage
        ("profile_para",       ["性情", "命格"]),                        # page_profile
        ("synthesis_para",     ["全局總論"]),                            # page_synthesis
        ("yang_zhai_para",     ["陽宅"]),                                # page_yangzhai
        ("ziwei_overall_para", ["全局總論", "命宮"]),                    # page_ziwei
    ]

    def _id_text(block):
        """Take detail_section block → render ready Indo HTML.
        block.id_summary is array of bullets — convert to HTML <ul> or single para."""
        if not isinstance(block, dict):
            return None
        ids = block.get("id_summary")
        if isinstance(ids, list) and ids:
            if len(ids) == 1:
                return _safe_html(ids[0])
            return "<ul>" + "".join(f"<li>{_safe_html(b)}</li>" for b in ids) + "</ul>"
        if isinstance(ids, str) and ids:
            return _safe_html(ids)
        return None

    nar_pairs = []
    for michele_key, hz_candidates in NAR_MAP_HZ:
        baseline = michele_para.get(michele_key)
        if not baseline:
            continue
        # Find first matching Hanzi key in subject's detail_sections
        block = None
        for hz_label in hz_candidates:
            if hz_label in detail:
                block = detail[hz_label]
                break
        txt = _id_text(block)
        if txt:
            nar_pairs.append((baseline, txt))

    # Prepend narrative pairs so they run first
    return nar_pairs + pairs
...
def _safe_html(text: str) -> str:
    """V4.6: HTML-escape user text but preserve a small whitelist of inline tags."""
...
def regen_dm_element_cards(subject: dict) -> str:
    ...
    yong_set = set((ys.get("elements_hz") or "").split())
    ji_set = set((js.get("elements_hz") or "").split())
...
def apply_blocks(content: str, subject: dict, fname: str = "") -> str:
    ...
    if wx:
        ...
        # Wu Xing tags (V4.6: yong/ji optional — skip block if either None)
        dm = subject["day_master"]
        ys = subject.get("yong_shen")
        js = subject.get("ji_shen")
        if ys and js:
    ...
    if da_yun:
        ...
        # V4.6: footer_caption_html removed — page_dayun footer wired via {{narrative.liu_nian.id}}
    ...
    if marriage and fname == "page_marriage.html":
        ...
        # V4.6: footer_caption_html removed — page_marriage footer wired via {{narrative.marriage.id}}
    ...
    if yang_zhai and fname == "page_yangzhai.html":
        ...
        # V4.6: footer_caption_html removed — page_yangzhai footer wired via {{narrative.yang_zhai.id}}

    # V4.6 FINAL: scrub residual Lin Ru Yi narrative paragraphs that didn't get subbed
    # (subject lacks the corresponding detail_section). Blank them out so master HTML
    # carries ZERO Lin Ru Yi narrative for new subject.
    content = _scrub_residual_lry(content)

    return content


def _scrub_residual_lry(content: str) -> str:
    """Replace any Lin Ru Yi baseline narrative still present with empty placeholder.
    Runs at end of apply_blocks per page, after narrative substitution."""
    michele_para = MICHELE.get("narrative_lin_ru_yi", {})
    placeholder = '<em style="opacity:.4">[Bagian ini tidak tersedia dari foto sumber.]</em>'
    for baseline in michele_para.values():
        if baseline and baseline in content:
            content = content.replace(baseline, placeholder)
    return content
...
# V4.6 FINAL: page → _page_status key mapping. None = always render.
# Status keys match build_from_ocr.py's auto-generated _page_status dict.
PAGE_TO_STATUS_KEY = {
    "page_01_cover.html":          None,
    "page_02_toc.html":            None,
    "page_03_intro.html":          None,
    "page_profile.html":           "profile",
    "page_05_bazi_opener.html":    None,
    "page_06_daymaster.html":      "daymaster",
    "page_marriage.html":          "marriage_compat",
    "page_08_xingqing.html":       "xingqing",
    "page_09_family.html":         "family",
    "page_10_shensha.html":        "shensha",
    "page_11_caifu.html":          "caifu",
    "page_career.html":            "career",
    "page_yangzhai.html":          "yangzhai",
    "page_dayun.html":             "dayun",
    "page_15_ziwei_opener.html":   "ziwei_opener",
    "page_ziwei.html":             "ziwei_chart",
    "page_17_palace1.html":        "career",
    "page_18_palace2.html":        "palace_marriage",
    "page_19_palace3.html":        "palace_fortune",
    "page_20_kesimpulan.html":     "kesimpulan",
    "page_synthesis.html":         "synthesis",
    "page_22_glossary.html":       None,
    "page_23_disclaimer.html":     None,
}

# Indo + Hanzi titles per page (for dynamic TOC reconstruction)
PAGE_TITLES = {
    "page_01_cover.html":        ("命",      "SAMPUL"),
    "page_02_toc.html":          ("目錄",    "DAFTAR ISI"),
    "page_03_intro.html":        ("引言",    "PENGANTAR"),
    "page_profile.html":         ("身份",    "PROFIL DIRI"),
    "page_05_bazi_opener.html":  ("八字",    "BA ZI"),
    "page_06_daymaster.html":    ("日主",    "PENGUASA HARI"),
    "page_marriage.html":        ("婚配",    "PASANGAN"),
    "page_08_xingqing.html":     ("性情",    "KEPRIBADIAN"),
    "page_09_family.html":       ("家庭",    "KELUARGA"),
    "page_10_shensha.html":      ("神煞",    "BINTANG NASIB"),
    "page_11_caifu.html":        ("財富",    "REZEKI"),
    "page_career.html":          ("事業",    "KARIR"),
    "page_yangzhai.html":        ("陽宅",    "RUMAH"),
    "page_dayun.html":           ("大運",    "DA YUN"),
    "page_15_ziwei_opener.html": ("紫微",    "ZI WEI"),
    "page_ziwei.html":           ("紫微盤",  "BAGAN ZI WEI"),
    "page_17_palace1.html":      ("命宮一",  "PALACE INTI"),
    "page_18_palace2.html":      ("命宮二",  "PALACE OPERASIONAL"),
    "page_19_palace3.html":      ("命宮三",  "PALACE WARISAN"),
    "page_20_kesimpulan.html":   ("總結",    "KESIMPULAN"),
    "page_synthesis.html":       ("合一",    "SINTESIS"),
    "page_22_glossary.html":     ("詞彙",    "GLOSARIUM"),
    "page_23_disclaimer.html":   ("免責",    "PENAFIAN"),
}
...
def _active_pages(subject) -> list[str]:
    """V4.6: returns pages to render based on subject._page_status."""
    status = subject.get("_page_status", {})
    out = []
    for fname in PAGE_ORDER:
        key = PAGE_TO_STATUS_KEY.get(fname)
        if key is None:
            out.append(fname)  # mandatory page
...
def render_pages(subject, out_dir):
    ...
    active = _active_pages(subject)
    print(f"[V4.6] rendering {len(active)}/{len(PAGE_ORDER)} pages "
          f"(skipped: {[f for f in PAGE_ORDER if f not in active]})")
...
def build_master(out_dir: Path, subject: dict) -> Path:
    ...
    # V4.6: only iterate active pages (rendered files exist) — pre-renumber + TOC inject.
    active = [f for f in PAGE_ORDER if (out_dir / f).exists()]
    total_pages = len(active)

    # V4.6: Build dynamic TOC HTML matching template design (toc-section + toc-rows).
    # Group active pages into 3 sections: Pendahuluan (1-4), Bagian I/II (5-19), Penutup (20+).
    PRELUDE = {"page_01_cover.html","page_02_toc.html","page_03_intro.html","page_profile.html"}
    BODY = {"page_05_bazi_opener.html","page_06_daymaster.html","page_marriage.html",
            "page_08_xingqing.html","page_09_family.html","page_10_shensha.html","page_11_caifu.html",
            "page_career.html","page_yangzhai.html","page_dayun.html","page_15_ziwei_opener.html",
            "page_ziwei.html","page_17_palace1.html","page_18_palace2.html","page_19_palace3.html"}
    OUTRO = {"page_20_kesimpulan.html","page_synthesis.html","page_22_glossary.html","page_23_disclaimer.html"}

    def _row(idx, fname):
        hz, indo = PAGE_TITLES.get(fname, ("·", fname))
        return f'          <div class="toc-row"><div class="pg">{idx:02d}</div><div class="ico">{hz[0] if hz else "·"}</div><div class="name"><strong>{hz}</strong>{indo}</div><div></div></div>'

    def _section(num_hz, title, id_label, rows):
        if not rows:
            return ""
        return ('        <div class="toc-section">\n'
                f'          <div class="num">{num_hz}</div>\n'
                f'          <div class="title">{title}<span class="id">{id_label}</span></div>\n'
                '        </div>\n'
                '        <div class="toc-rows">\n'
                + "\n".join(rows) + '\n        </div>\n')

    prelude_rows, body_rows, outro_rows = [], [], []
    for idx, fname in enumerate(active, start=1):
        row = _row(idx, fname)
        if fname in PRELUDE:    prelude_rows.append(row)
        elif fname in BODY:     body_rows.append(row)
        elif fname in OUTRO:    outro_rows.append(row)

    toc_inner = (
        _section("序", "序 言", f"Pendahuluan · Pages 1–{len(prelude_rows)}", prelude_rows)
        + _section("壹", "八字 / 紫微", f"Bagian Inti · Pages {len(prelude_rows)+1}–{len(prelude_rows)+len(body_rows)}", body_rows)
        + _section("叁", "總 結", f"Penutup · Pages {len(prelude_rows)+len(body_rows)+1}–{total_pages}", outro_rows)
    )
    toc_dynamic = "\n" + toc_inner
    ...
    for idx, fname in enumerate(active, start=1):
        ...
        # V4.6: page renumber footer "Halaman X / 23" → "Halaman {idx} / {total_pages}"
        content = re.sub(r'Halaman\s+\d+\s*/\s*\d+', f'Halaman {idx} / {total_pages}', content)

        # V4.6: dynamic TOC injection — replace block between markers if present, else
        # find the Lin Ru Yi static toc-list and replace it.
        if fname == "page_02_toc.html":
            # Marker-based first
            new_content, n = re.subn(
                r'<!--\s*TOC_AUTO\s*-->.*?<!--\s*/TOC_AUTO\s*-->',
                f'<!--TOC_AUTO-->{toc_dynamic}<!--/TOC_AUTO-->',
                content, count=1, flags=re.DOTALL)
            content = new_content if n else content
...
"""
    ...
    # V4.6 C2: SAFETY SCAN — detect Lin Ru Yi / 林如意 leftover that's NOT in subject's allowlist.
    safety_scan(master, subject)

    return master_path


# Lin Ru Yi (Michele baseline) leak detection patterns. Run AFTER all subs.
_LRY_LEAK_PATTERNS = [
    "Lin Ru Yi", "林如意",
    "辛金 Anda adalah",       # Lin Ru Yi day-master narrative line
    "正官格",                  # Lin Ru Yi format (only flag if not subject's format)
    "亥 Babi",                # Lin Ru Yi shio
    "Pasangan ber-shio Kelinci",   # Lin Ru Yi marriage para
    "pedang belum ditempa",    # Lin Ru Yi personality
    "Beri 5-10% rezeki",      # Lin Ru Yi wealth
    "Logam yang ditempa Api siap jadi pedang",  # Lin Ru Yi kesimpulan
    "30 Mei 1995",            # Lin Ru Yi birthdate
    "1995-05-30",             # ISO form
    "民國 84",                # Lin Ru Yi RoC year
]


def safety_scan(master_html: str, subject: dict) -> None:
    """V4.6: scan rendered master for Lin Ru Yi leak. Build allowlist of subject's
    own values (e.g. if subject's format IS 正官格, don't flag it). Raise on leak.
    Strip HTML comments before scanning (template SVG comments are not user-visible)."""
    # Strip HTML comments to avoid false positives from labels in SVG/template comments
    scan_html = re.sub(r"<!--.*?-->", "", master_html, flags=re.DOTALL)

    iden = subject.get("identity", {})
    fmt = subject.get("format", {})
    formats = subject.get("formats", [])
    shio = subject.get("shio", {})
    pillars = subject.get("pillars", {})
    dm = subject.get("day_master", {})

    allow = set()
    if iden.get("name_id"):  allow.add(iden["name_id"])
    if iden.get("name_hanzi"): allow.add(iden["name_hanzi"])
    if iden.get("birth_date"): allow.add(iden["birth_date"])
    if fmt.get("hz"): allow.add(fmt["hz"])
    for f in formats or []:
        if f.get("hz"): allow.add(f["hz"])
    if shio.get("id"): allow.add(shio["id"])
    if shio.get("branch_hz"): allow.add(shio["branch_hz"])
    for k in ("year","month","day","hour"):
        p = pillars.get(k) or {}
        if p.get("stem_hz"): allow.add(p["stem_hz"])
        if p.get("branch_hz"): allow.add(p["branch_hz"])
    if dm.get("stem_hz"): allow.add(dm["stem_hz"])

    leaks = []
    for pat in _LRY_LEAK_PATTERNS:
        if pat in scan_html and pat not in allow:
            idx = scan_html.find(pat)
            ctx = scan_html[max(0, idx-40):idx+len(pat)+40].replace("\n", " ")
            leaks.append((pat, ctx))

    if leaks:
        msg = f"\n[V4.6 SAFETY] Lin Ru Yi leak detected ({len(leaks)} pattern):\n"
        for pat, ctx in leaks[:10]:
            msg += f"  - '{pat}' near: …{ctx}…\n"
        msg += "Subject allowlist: " + ", ".join(sorted(allow)) + "\n"
        msg += "Re-shoot foto untuk section yang fail, atau adjust template Lin Ru Yi baseline."
